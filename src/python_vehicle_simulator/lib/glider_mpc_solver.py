...
Author: MPC implementation for Seawing Glider
"""
...
class GliderMPCController:
    ...
    def compute_control(self, eta, nu, u_actual, sampleTime):
        ...
        # 4. 积分得到位置指令（使用内部积分状态，避免噪声累积）
        self.u_integrated += u_dot_opt * sampleTime
        
        # 5. 抗饱和：如果指令超限，截断并反向修正积分状态
...
if __name__ == "__main__":
    ...
    x0 = np.zeros(15)
    x0[6] = 0.1  # V1 = 0.1 m/s
    x0[12] = 0.4  # r_rx = 0.4 m
    
    x_ref = config.get_dive_reference()
    
    print("\nSolving MPC problem...")
    print(f"Initial state: x0 = {x0}")
    print(f"Reference: x_ref = {x_ref}")
    
    u_opt = solver.solve(x0, x_ref)
    
    print(f"\nOptimal control: u_opt = {u_opt}")
    print(f"Solve time: {solver.solve_time:.4f} s")
    print(f"Success: {solver.solve_success}")
    
    # 获取预测轨迹
    X_opt, U_opt = solver.get_predicted_trajectory()
    if X_opt is not None:
        print(f"\nPredicted trajectory shape: X={X_opt.shape}, U={U_opt.shape}")
        print(f"Predicted theta at k=0: {np.degrees(X_opt[4, 0]):.2f} deg")
        print(f"Predicted theta at k=N: {np.degrees(X_opt[4, -1]):.2f} deg")
    
    print("\nGliderMPCSolver test completed!")
